...
class Owner:
    ...
    async def _set(self, ctx):
        """Changes riceBots's global settings."""
    ...
    async def owner(self, ctx):
        ...
        if self.bot.settings.no_prompt is True:
            await self.bot.say("Console interaction is disabled. Start riceBot "
    ...
    async def prefix(self, ctx, *prefixes):
        """Sets riceBot's global prefixes
    ...
    async def serverprefix(self, ctx, *prefixes):
        """Sets riceBot's prefixes for this server
        Accepts multiple prefixes separated by a space. Enclose in double
        quotes if a prefix contains spaces.
    ...
    async def stream(self, ctx, streamer=None, *, stream_title=None):
        """Sets riceBot's streaming status
    ...
    async def restart(self, silently : bool=False):
        """Attempts to restart riceBot
        Makes riceBot quit with exit code 26
